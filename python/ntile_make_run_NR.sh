...
cp precompile_cu_files/NR_test_${1}_preprocess_RK2_kernel.cu ../cuda/RK2/kernel.cu
cd ../cuda
pwd
## build the new ODE system and link to the solver .so 's
make
cd ../python
python python_harness.py --Ntile=$1 --katz=False --NR=True --SIE=True --PY=False --RK2=True --SIM=True
