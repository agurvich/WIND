...
from chimes_driver.utils.chimes_classes import globalVariables
...
class FullChimes(ODEBase):
    def __init__(
        ...
        **kwargs):
        ...
        self.eqn_labels = [
            ...
            ['H0','H+',"He0","He+","He++"]] ## TODO decide how many abundances i'm sending it
    ...
    def preflight(self):
        ...
        my_driver = ChimesDriver(
            self.nH_arr, self.temperature_arr, self.metallicity_arr, self.shieldLength_arr, 
            self.init_chem_arr, 
            self.driver_pars, self.global_variable_pars, self.gas_variable_pars,
            rank = 0,
            init_network=False, ## don't init the network
            init_gas_vars=False) ## don't init the gas variables


        self.init_wind_chimes.argtypes = [ctypes.POINTER(globalVariables)]

        #for key in dir(my_driver.myGlobalVars):
            #if 'Path' in key:
                #print(key,getattr(my_driver.myGlobalVars,key))

        print("Initializing WIND CHIMES...")
        self.init_wind_chimes(my_driver.myGlobalVars)
    ...
    def calculate_derivative(self,system_index=0):
        raise NotImplementedError("This is hard to replicate for Full Chimes, no derivative!")
        if type(system_index) == int:
            equations = self.equations[
                system_index*self.Neqn_p_sys:
                (system_index+1)*self.Neqn_p_sys]
    ...
    def calculate_eqmss(self):
        ## TODO assumes Katz' equilibriums but that might be okay
        ##  strictly we should read the rate coefficients from CHIMES
        ##  but this is equivalent to setting the creation and destruction
        ##  rates equal which would require calculating the rates...
        helium_mass_fractions = self.metallicity_arr[:,1]
        y_heliums = helium_mass_fractions / (4*(1-helium_mass_fractions))
...
### PRECOMPILE STUFF FOR MAKING .CU FILES
    def make_jacobian_block(self,this_tile,Ntile):
        raise NotImplementedError("No precompiling for full chimes, never again")
